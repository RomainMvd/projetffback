...
public interface EnseignantRepository extends JpaRepository<Enseignant, Long> {
	...
	@Query(value = "select * from personne where personne_type='enseignant' and (id_personne IN (select id_personne from lecture where id_cours IN (select id_cours from lecture where id_personne=?1)) OR id_personne IN (select id_personne from personne_classe where id_classe IN (select id_classe from personne_classe where id_personne=?1)))", nativeQuery = true)
	List<Enseignant> afficherEnseignantsEtudiant(String idEtudiant);

	@Query(value = "select * from personne where personne_type='enseignant' and (id_personne IN (select id_personne from lecture where id_cours IN (select id_cours from lecture where id_personne=?1)) OR id_personne IN (select id_personne from personne_classe where id_classe IN (select id_classe from personne_classe where id_personne=?1))) and id_personne IN (select id_personne from profil where id_role=?2)", nativeQuery = true)
	...
	@Query(value = "select * from personne where id_personne=?2 and personne_type='enseignant' and (id_personne IN (select id_personne from lecture where id_cours IN (select id_cours from lecture where id_personne=?1)) OR id_personne IN (select id_personne from personne_classe where id_classe IN (select id_classe from personne_classe where id_personne=?1)))", nativeQuery = true)
	Enseignant afficherEnseignantEtudiant(String idEtudiant, String idEnseignant);

	@Query(value = "select * from personne where id_personne=?3 and personne_type='enseignant' and (id_personne IN (select id_personne from lecture where id_cours IN (select id_cours from lecture where id_personne=?1)) OR id_personne IN (select id_personne from personne_classe where id_classe IN (select id_classe from personne_classe where id_personne=?1))) and id_personne IN (select id_personne from profil where id_role=?2)", nativeQuery = true)
